...
通用 ASR 模型对 Kafka、Flink、WebRTC 等专业词，以及课程名词、产品名和人名的识别效果不稳定。
...
当前 Kafka、Flink、Redis、ASR 和 API 都通过 Docker Compose 部署在一台机器上。它适合课程设计和本地实验，但不能直接代表生产环境的高可用集群。
...
| HTML 报告 | 汇总字幕、关键词、延迟曲线、失败片段和实验结论。 | 老师打开一个页面即可理解实验结果。 |
