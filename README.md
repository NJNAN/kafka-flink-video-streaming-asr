...
| **交付形态完整** | 只能命令行跑脚本 | Web Dashboard、离线 Electron 工作台、实时 Electron 采集端、平板会议纪要 App | `desktop-ui/`、`desktop-ui-live/`、`meeting-assistant-tablet/` |
| **面向真实场景扩展** | 一次性脚本到此为止 | 支持会议、技术讲解、大数据等领域 Profile；支持热词确认/忽略/纠错；支持后续 RTSP/多 ASR worker 扩展 | `config/profiles/`、`/api/hotwords/action`、`docs/问题解决与拓展路线.md` |
...
| 运行环境 | Docker Compose 一键启动，前后端与桌面端入口完整 | 第 3 节 |
...
| 文档体系 | 配套文档、问题解决、拓展路线、评测说明 | 第 9 节 |
...
| 11 |           [领域Profile说明.md](./docs/领域Profile说明.md) | 如何切换不同领域（大数据/会议/技术讲解）的词表 |
...
建议按“数据流”验证系统，几分钟内把核心链路跑通并看到可量化结果：
...
8. **体验 MeetFlow 平板端**：验证同一条实时语音链路可以扩展为会议纪要产品。

一句话概括：

> StreamSense 把视频语音转写从“离线脚本”升级成“大数据实时管线”。Kafka 负责缓冲和解耦，Flink 负责流式调度，ASR 负责本地推理，FastAPI 负责聚合分析，Redis/SQLite/JSONL 负责实时展示和结果追溯，最终通过 Dashboard、桌面端、平板端和自动化测试形成完整的工程闭环。
