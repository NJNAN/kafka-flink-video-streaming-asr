...
| **大数据课程知识点闭环** | 只展示模型识别结果 | Kafka 解耦、Flink 调度、Redis 缓存、SQLite 统计、Docker Compose 编排全部落地 | `docker-compose.yml`、`flink/transcription_job.py`、`services/api/app.py` |
| **可观测与可复盘** | 看不到中间过程 | 记录端到端延迟、ASR 耗时、调度耗时、吞吐增量、失败片段和历史采样 | `data/results/*.jsonl`、`/api/database/summary`、`docs/system_metrics.md` |
| **质量不靠感觉** | “字幕看起来还行” | CER/WER、关键词命中率、字幕条数、缺口数、耗时/视频时长全部量化 | `tools/evaluate_subtitles.py`、`docs/字幕质量评测说明.md` |
| **演示形态完整** | 只能命令行跑脚本 | Web Dashboard、离线 Electron 工作台、实时 Electron 采集端、华为平板会议纪要 App | `desktop-ui/`、`desktop-ui-live/`、`meeting-assistant-tablet/` |
| **面向真实场景扩展** | 课设结束即停止 | 支持会议、课程、大数据等领域 Profile；支持热词确认/忽略/纠错；支持后续 RTSP/多 ASR worker 扩展 | `config/profiles/`、`/api/hotwords/action`、`docs/问题解决与拓展路线.md` |
...
| 演示数据一键清理 | 直播/答辩前可清空某一路历史，不用手动删 JSONL | `DELETE /api/streams/{stream_id}/segments` |
| Live Ingest 降噪与低音量优化 | 平板/笔记本麦克风声音偏小时仍能接入实时链路 | `LIVE_INGEST_MIN_DBFS=-55`、`LIVE_INGEST_MIN_WAV_BYTES=24000` |
| MeetFlow 平板会议纪要 | 把实时语音链路包装成“按一下开始记录会议”的可用产品 | `meeting-assistant-tablet/`、Capacitor Android 构建 |
...
这组截图来自 `meeting-assistant-tablet/` 的实际界面：项目不仅有 Kafka/Flink 后端，也有面向真实会议场景的前端交付。平板端负责采集语音片段，后端仍走 StreamSense Live 链路，最终把实时转写结果整理为会议摘要、待办和原文摘录。

## 课程验收点对照

| 课程验收点 | README 中可直接看到的内容 | 对应文件 |
| --- | --- | --- |
| 案例选型 | 视频流语音转写天然是流式数据处理场景 | 第 1 节 |
| 技术选型 | Kafka、Flink、Redis、FastAPI、Docker、ASR、VAD 分层说明 | 第 2 节 |
| 开发环境 | Docker Compose 一键启动，前后端与桌面端入口完整 | 第 3 节 |
| 核心技术 | 架构图、时序图、Topic 设计、数据规格 | 第 4-5 节 |
| 功能实现 | 实时字幕、关键词、热词、导出、桌面端、会议纪要、测试 | 第 6 节 |
| 代码规范 | 模块拆分、目录结构、配置外置、关注点分离 | 第 7 节 |
| 实验结果 | 2 路/4 路并发压测、失败数、平均延迟、P95 延迟 | 第 8 节 |
| 说明文档 | 14+ 篇配套文档、问题解决、拓展路线、评测说明 | 第 9 节 |
...
| 使用平板会议纪要    | `cd meeting-assistant-tablet && npm run dev:https`，华为平板同 Wi-Fi 访问 Vite 局域网地址                         |
| 查看历史指标曲线    | `http://localhost:8000/api/metrics/history`，支持 `?stream_id=desktop-live`                                       |
| 清空一路演示数据    | `DELETE http://localhost:8000/api/streams/{stream_id}/segments`，直播演示前可重置                                 |
...
| **Capacitor**        | 8    | 将会议纪要 Web App 打包为 Android APK | 移动端封装   |
| **PWA / HTTPS Dev**  | -    | 平板浏览器获取麦克风权限、添加到桌面运行 | 移动端体验   |
...
| **平板会议纪要**        | 麦克风分段上传，实时转写后自动生成摘要和待办       | `meeting-assistant-tablet/`              |
...
├── meeting-assistant-tablet/       # 华为平板 / Android 会议纪要端
...
| ------------------------ | -------- | ---- | -------: | -------: | ---- |
| **2 路视频并发**         | 230      | 0    |  ~4.1 秒 |  ~6.8 秒 | 推荐作为稳定基准 |
| 4 路视频并发             | 39       | 0    |  ~9.9 秒 | ~14.5 秒 | 排队延迟增加，仍可稳定运行 |

> 2 路视频并发结果零失败、平均延迟约 4 秒、P95 延迟约 6.8 秒，适合作为课程报告和系统演示中的主基准数据。
...
这部分的目标是说明：项目不是只“能跑一次”，而是有可重复验证路径。测试分为四层，覆盖从纯函数到完整大数据链路。
...
| `tests/test_smoke_report.py` | 冒烟测试结果行生成、Markdown 报告输出 | 证明验收报告格式稳定，便于答辩前留痕 |
...
| 形态 | 验收命令 | 可观察结果 |
| --- | --- | --- |
| Web Dashboard | `docker compose up -d --build` 后打开 `http://localhost:8000` | 实时字幕、关键词、延迟、吞吐、失败片段 |
| 离线桌面端 | `cd desktop-ui && npm run electron:dev` | 选择视频、生成字幕、质量报告、导出 ZIP |
| 实时桌面端 | `cd desktop-ui-live && npm run electron:dev` | 麦克风/摄像头采集走 Kafka-Flink 实时链路 |
| 平板会议纪要 | `cd meeting-assistant-tablet && npm run dev:https` | 一键记录会议、实时文字、摘要、待办 |
| Android APK | `cd meeting-assistant-tablet && npx cap sync android && .\android\gradlew.bat -p android assembleDebug` | 本地生成 debug APK，构建产物不提交到仓库 |

---

## 13. 答辩演示路线

建议演示时按“数据流”讲，3 分钟内把课程知识点串起来：

1. **先打开 README 首屏**：指出系统不是单一字幕脚本，而是 Kafka-Flink 实时处理链路，并展示 GIF。
2. **启动 Docker Compose**：说明 Kafka、Flink、Redis、ASR、API、Dashboard 是独立服务。
3. **打开 Dashboard**：展示实时字幕、关键词、延迟曲线和失败片段区域。
4. **打开 Flink Web UI**：说明 Flink 负责消费 Kafka 片段并调度 ASR。
5. **展示导出接口**：用 `/api/streams/{stream_id}/export?format=srt` 证明结果可落地成字幕文件。
6. **运行验收脚本**：用 `python tools/smoke_check.py` 证明服务状态不是口头描述。
7. **展示评测报告**：用 CER/WER/关键词命中率说明质量可量化。
8. **最后展示 MeetFlow 平板端**：说明同一条实时语音链路可以扩展为会议纪要产品。

总结表达：

> 本项目把视频语音转写从“离线脚本”升级成“大数据实时管线”。Kafka 负责缓冲和解耦，Flink 负责流式调度，ASR 负责本地推理，FastAPI 负责聚合分析，Redis/SQLite/JSONL 负责实时展示和结果追溯。最后通过 Dashboard、桌面端、平板端和自动化测试，把处理过程、实验结果和实际应用完整展示出来。
